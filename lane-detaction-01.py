...
def warp(image):
    ...
    src = np.float32([[137 ,152], [15 ,212], [297 ,157], [455 ,303]])
...
def lane_pixels(threshold_image):
    ...
    for window in range(window_number):
        ...
        window_yhigh = h - window * window_height
        window_xleft_low = current_lwindow - window_width // 2
        window_xleft_high = current_lwindow + window_height // 2
        window_xright_low = current_rwindow - window_width // 2
        window_xright_high = current_rwindow + window_height // 2

        selected_pixels_left = ((nonzero_y >= window_ylow) & (nonzero_y < window_yhigh) & (nonzero_x >= window_xleft_low) & (nonzero_x < window_xleft_high)).nonzero()[0]
        selected_pixels_right = ((nonzero_y >= window_ylow) & (nonzero_y < window_yhigh) & (nonzero_x >= window_xright_low) & (nonzero_x < window_xright_high)).nonzero()[0]
    ...
    return left_x, left_y, right_x, right_y, histogram
...
def find_curvature(left_fit_x, right_fit_x, y_values, image):
    ...
    left_curvature = ((1 + (2 * left_fit_cv[0] * y_eval * ym_per_pix + left_fit_cv[1]) ** 2) ** 1.5) / np.absolute(2 * left_fit_cv[0])
    right_curvature = ((1 + (2 * right_fit_cv[0] * y_eval * ym_per_pix + right_fit_cv[1]) ** 2) ** 1.5) / np.absolute(2 * right_fit_cv[0])

    left_slope = 2 * left_fit_cv[0] * y_eval * ym_per_pix + left_fit_cv[1]
    right_slope = 2 * right_fit_cv[0] * y_eval * ym_per_pix + right_fit_cv[1]
...
left_x, left_y, right_x, right_y, histogram = lane_pixels(threshold_image)
...
# Keep the window open until the user presses 'q' to quit
while True:
